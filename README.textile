...
Micro-library providing programmatic access to "Colorado Transparency in Contribution and Expenditure Reporting (TRACER)":http://tracer.sos.colorado.gov/PublicSite/homepage.aspx campaign finance data.
...
Although Colorado enjoys excellent campaign finanance reporting, the state publishes that data through single file archives with CSV reports. The Colorado Secretary of State Elections Divison posts those three types of reports (contribution, expenditure, and loan data) through the "Colorado Transparency in Contribution and Expenditure Reporting (TRACER)":http://tracer.sos.colorado.gov/PublicSite/homepage.aspx system. This micro-library provides programmatic access to that system through automated URL generation, file download, archive extraction, and data interpretation.


h2. Full API

See the api_doc/index.html file in the api_doc subdirectory.
...
This pure-Python micro-library uses the Python standard library and does not require the installation of additional modules. That being said, some unit tests use the "pymox":https://code.google.com/p/pymox/ library for dependency injection and, while not including the library directly, mongo_aggregator operates on structures from "pymongo":http://api.mongodb.org/python/current/.
